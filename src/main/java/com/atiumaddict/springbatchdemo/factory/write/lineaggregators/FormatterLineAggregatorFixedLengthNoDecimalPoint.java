 ...
 */
@Slf4j
public class FormatterLineAggregatorFixedLengthNoDecimalPoint<T> extends FormatterLineAggregator {
    ...
    public FormatterLineAggregatorFixedLengthNoDecimalPoint() {
    }
    ...
    protected String doAggregate(Object[] fields) {
        ...
        for (int i = 0; i < fields.length; i++) {
            if (fields[i] instanceof LocalDateTime) {
                ...
                fields[i] = Long.valueOf(String.format("%.2f", fields[i]).replace(".", "").replace(",", ""));
            }
        }
